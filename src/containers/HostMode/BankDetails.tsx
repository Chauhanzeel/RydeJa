...
const BankDetails: React.FC<FilterModalProps> = ({ route }) => {
  ...
  const [isLoading, updateBankDetailsData, profileDetailsData] = useSelector(
    (state: ReducerStateProps) => [
      state.payment.isLoading,
      state.payment.updateBankDetailsData,
  ...
  const [accountType, setAccountType] = useState(
    _.get(profileDetailsData, "verifiedInfo.bankDetails[0].accountType", null)
  );
  const [accountTypeName, setAccountTypeName] = useState(null);
  const [routingNumberError, setRoutingNumberError] = useState(0);
  const [routingNumber, setRoutingNumber] = useState(
    _.get(profileDetailsData, "verifiedInfo.bankDetails[0].routingNumber", null)
  );
  ...
  const [accountNumber, setAccountNumber] = useState(
    _.get(profileDetailsData, "verifiedInfo.bankDetails[0].accountNumber", null)
  );
  const [bankNameError, setBankNameError] = useState(0);
  const [bankName, setBankName] = useState(
    _.get(profileDetailsData, "verifiedInfo.bankDetails[0].bankName", null)
  );
  const [branchError, setBranchError] = useState(0);
  const [branch, setBranch] = useState(
    _.get(profileDetailsData, "verifiedInfo.bankDetails[0].branch", null)
  );
  const [accountError, setAccountError] = useState(0);
  const [update, setUpdate] = useState(false);

  useEffect(() => {
    if (_.get(updateBankDetailsData, "success", null) == true && update) {
      goBack();
      setUpdate(false);
    }
  ...
  const deposit = () => {
    ...
    if (
      ...
      dispatch(
        updateBankDetailsStart(
          data,
          _.get(profileDetailsData, "verifiedInfo.bankDetails[0].id", null)
        )
      );
      setUpdate(true);
    }
