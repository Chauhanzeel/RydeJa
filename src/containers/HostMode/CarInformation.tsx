...
import { carInfoListStart } from "../../actions/carOwnerActions";
...
const CarInformation: React.FC<CancelProps> = ({ route }) => {
  const { t } = useTranslation();

  const dispatch = useDispatch();
  const [isLoading, carInfoListData, ownerCarViewData] = useSelector(
    (state: ReducerStateProps) => [
      state.carOwner.isLoading,
      state.carOwner.carInfoListData,
  ...
  useEffect(() => {
    if (ownerCarViewData) {
      const id = _.get(ownerCarViewData, "carInformation", []).map(
        (item: { id: any }) => item.id
      );
      setSelectedId(id);
    }
  ...
  const renderCarInfoData = (item: any, i: any) => {
    return (
  ...
  const storeData = async () => {
    if (_.size(selectedId) > 0) {
      ...
      parsedData.carInformations = selectedId;
      ...
      >
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.featuresView}>
            ...
            >
              What car information does your Ryde have?
            </Text>
            {!isLoading ? (
              <>
                {_.size(_.get(carInfoListData, "items", null)) > 0 && (
                  <View
                    style={{
                  ...
                  >
                    <FlatList
                      data={_.get(carInfoListData, "items", null) || []}
                      renderItem={({ item, i }: any) =>
                        renderCarInfoData(item, i)
                      }
                      numColumns={2}
                      nestedScrollEnabled
                    />
